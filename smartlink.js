...
function getRandomSmartLink() {
  ...
  do {
    index = Math.floor(Math.random() * SMART_LINKS.length);
  } while (index === lastIndex && SMART_LINKS.length > 1);

  lastIndex = index;
  return SMART_LINKS[index];
}

/* SMART GATE SYSTEM (GLOBAL ROUTER) */
const ACCESS_TIME = 10 * 60 * 1000; // 10 minutes unlock

export function openSmartGate(targetPage) {
  ...
  localStorage.setItem("target_page", targetPage);
  localStorage.setItem("awaiting_return", "yes");

  // Open smart link
  window.open(getRandomSmartLink(), "_blank");
...
window.addEventListener("focus", () => {
  ...
  if (waiting === "yes") {
    ...
    const expires = Date.now() + ACCESS_TIME;
...
/* OPTIONAL EXPORT FOR DEBUG OR FUTURE USE */
export function getRandomSmartLink() {
